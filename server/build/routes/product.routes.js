...
router.get("/srapFromAsin/:asin", withAuth(Role.SELLER), asyncHandler(async (request, reply) => {
...
router.post("/create", withAuth(Role.SELLER), asyncHandler(async (request, reply) => {
    ...
    const { item } = zodValidationForRoute(request.body, z.object({
...
router.get("/categories", asyncHandler(async (request, reply) => {
    const res = await ProductController.getCategories();
    reply.send(handleResponseForRoute(res, {
        missing_categories: new NotFoundRequestError("missing_categories"),
    }));
}));
router.get("/:productId", asyncHandler(async (request, reply) => {
    const { productId } = zodValidationForRoute(request.params, z.object({ productId: z.string() }));
    const res = await ProductController.getOne({ productId });
    reply.send(handleResponseForRoute(res, {
        not_found: new NotFoundRequestError("not_found"),
    }));
}));
router.post("/update", withAuth(Role.SELLER), asyncHandler(async (request, reply) => {
...
router.delete("/:productId", withAuth(Role.SELLER), asyncHandler(async (request, reply) => {
