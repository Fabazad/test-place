...
const from_email = configs.FROM_MAIL_ADDRESS;
const sgMail = require("@sendgrid/mail");
sgMail.setApiKey(configs.SENDGRID_API_KEY);
const senfGrid = require("sendgrid")(configs.SENDGRID_API_KEY);
const baseUrl =
...
class EmailController {
  ...
  static async sendValidateMailAddressMail(email, userId, language) {
    return this.sendEmail(
      email,
      from_email,
      MAIL_TEMPLATES_IDS.VALIDATE_EMAIL[language],
      { userId, baseUrl }
    );
  }
  ...
  static async sendResetPasswordMail(email, resetPasswordToken, language) {
    return this.sendEmail(
      email,
      from_email,
      MAIL_TEMPLATES_IDS.RESET_PASSWORD[language],
      { resetPasswordToken, baseUrl }
    );
  }
  ...
  static async sendNotificationMail(notification) {
    ...
    if (!toUser || !toUser.email) {
      return Promise.reject({
        status: 500,
        message: "Couldn't find the destination user email notification.",
      });
    }
    ...
    if (notification.type === NOTIFICATION_TYPES.TEST_CANCELLED.value) {
      ...
      adminParams.text =
        "test._id: " + notification.test._id.toString() + ". " + adminParams.text;
